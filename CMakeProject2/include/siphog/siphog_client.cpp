...
SIPHOGClient::SIPHOGClient()
    ...
    , m_isConnected(false)
    , m_isPolling(false)
    , m_pollingIntervalMs(1)  // CHANGED: Set to 1ms for maximum capacity
    ...
    , m_name("SIPHOG Controller")
{
    ...
    Logger::GetInstance()->LogInfo("SIPHOGClient: Initialized with maximum capacity polling");

    // Auto-connect to SIPHOG server on startup
    if (Connect("127.0.0.22", 22222)) {
        Logger::GetInstance()->LogInfo("SIPHOGClient: Auto-connected to SIPHOG server");
    }
    else {
        Logger::GetInstance()->LogWarning("SIPHOGClient: Auto-connect failed, will retry in Update()");
    }
}
...
bool SIPHOGClient::Connect(const std::string& ip, int port) {
    ...
    // OPTIMIZED: Increase socket buffer sizes for maximum throughput
    int bufferSize = 65536; // 64KB buffer
    if (setsockopt(m_socket, SOL_SOCKET, SO_RCVBUF, (const char*)&bufferSize, sizeof(bufferSize)) < 0) {
        logger->LogWarning("SIPHOGClient: Could not set receive buffer size");
    }
...
void SIPHOGClient::StartPolling() {
    ...
    Logger::GetInstance()->LogInfo("SIPHOGClient: Started maximum capacity polling");
}
...
void SIPHOGClient::SetPollingInterval(int intervalMs) {
    // REMOVED: No longer allow changing polling interval - always use maximum capacity
    // Keep minimum at 1ms for maximum throughput
    m_pollingIntervalMs = 1;
}
...
void SIPHOGClient::PollingThreadFunction() {
    Logger::GetInstance()->LogInfo("SIPHOGClient: Maximum capacity polling thread started");

    while (m_isPolling && m_isConnected) {
        if (FetchLatestData()) {
            // Successfully got fresh data - continue immediately for maximum throughput
        }

        // OPTIMIZED: Minimal sleep for maximum capacity (1ms)
...
bool SIPHOGClient::FetchLatestData() {
    ...
    try {
        ...
        timeout.tv_sec = 0;
        timeout.tv_usec = 100000; // CHANGED: 100ms timeout (was 500ms)
        ...
        char buffer[4096]; // OPTIMIZED: Larger buffer for better performance

        auto startTime = std::chrono::steady_clock::now();
        const auto maxWaitTime = std::chrono::milliseconds(100); // CHANGED: Reduced from 500ms to 100ms
        ...
        while (std::chrono::steady_clock::now() - startTime < maxWaitTime) {
            ...
            if (bytesReceived > 0) {
                ...
                // OPTIMIZED: Process all complete messages in buffer
                size_t searchPos = 0;
                while (searchPos < tempBuffer.length()) {
                    size_t startPos = tempBuffer.find("$START,", searchPos);
                    if (startPos != std::string::npos) {
                        ...
                        if (endPos != std::string::npos) {
                            ...
                            if (ParseAndStoreMessage(completeMessage)) {
                                // Continue processing more messages if available
                                searchPos = endPos + 5; // Move past ",END$"
                                continue;
                            }
                        }
                    }
                ...
                // If we processed messages, clean up the buffer
                if (searchPos > 0) {
                    tempBuffer = tempBuffer.substr(searchPos);
                    return true; // Successfully processed at least one message
                }
            }
...
void SIPHOGClient::StoreDataInGlobalStore(const SIPHOGData& data) {
    ...
    // Also store individual components with shorter keys for easy access
    store->SetValue("SIPHOG_SLED_Current", data.sledCurrent);
    store->SetValue("SIPHOG_Photo_Current", data.photoCurrent);
    store->SetValue("SIPHOG_SLED_Temp", data.sledTemp);
    store->SetValue("SIPHOG_Target_SAG_PWR", data.targetSagPwr);
    store->SetValue("SIPHOG_SAG_PWR", data.sagPwr);
    store->SetValue("SIPHOG_TEC_Current", data.tecCurrent);

    // Store connection status and timestamp
    store->SetValue("SIPHOG_Connected", m_isConnected ? 1.0f : 0.0f);
    store->SetValue("SIPHOG_Data_Age", std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now() - data.timestamp).count() / 1000.0f);
}
...
void SIPHOGClient::RenderUI() {
    ...
    // Connection controls
    static char ipBuffer[64] = "127.0.0.22";  // Updated default IP
    static int portBuffer = 22222;            // Updated default port
    ...
    // SIMPLIFIED: Polling controls - no interval adjustment
    ImGui::Text("Data Reception: %s", m_isPolling ? "Maximum Capacity" : "Stopped");
    ImGui::TextColored(ImVec4(0, 1, 0, 1), "Optimized for real-time data");

    if (m_isConnected) {
        if (!m_isPolling) {
            if (ImGui::Button("Start Reception")) {
                StartPolling();
            }
        }
        else {
            if (ImGui::Button("Stop Reception")) {
                StopPolling();
            }
        }
    }
    ...
    // Buffer info with performance indicators
    {
        std::lock_guard<std::mutex> lock(m_dataMutex);
        ImGui::Text("Buffer: %zu/%zu samples", m_dataBuffer.size(), m_maxBufferSize);

        // Show buffer fill percentage
        float fillPercentage = (float)m_dataBuffer.size() / (float)m_maxBufferSize * 100.0f;
        ImGui::ProgressBar(fillPercentage / 100.0f, ImVec2(-1, 0),
            (std::to_string((int)fillPercentage) + "%").c_str());
    }
...
void SIPHOGClient::Update() {
    // Main loop update method for when UI is hidden
    // Handles auto-reconnection and ensures data flows to global store
    ...
    // Auto-reconnection logic (try every 5 seconds)
    if (!m_isConnected) {
        if (std::chrono::duration_cast<std::chrono::seconds>(now - lastReconnectAttempt).count() >= 5) {
            Logger::GetInstance()->LogInfo("SIPHOGClient: Attempting auto-reconnection...");
            if (Connect("127.0.0.22", 22222)) {
                Logger::GetInstance()->LogInfo("SIPHOGClient: Auto-reconnection successful");
            }
            else {
                Logger::GetInstance()->LogWarning("SIPHOGClient: Auto-reconnection failed, will retry in 5 seconds");
            }
            lastReconnectAttempt = now;
        }

        // Update global store with disconnected status
        GlobalDataStore* store = GlobalDataStore::GetInstance();
        store->SetValue("SIPHOG_Connected", 0.0f);
        store->SetValue("SIPHOG_Data_Age", 999.0f); // Large value indicates stale data
    }

    // Status logging (every 30 seconds when connected)
    if (m_isConnected && std::chrono::duration_cast<std::chrono::seconds>(now - lastStatusUpdate).count() >= 30) {
        size_t bufferSize;
        {
            std::lock_guard<std::mutex> lock(m_dataMutex);
            bufferSize = m_dataBuffer.size();
        }

        SIPHOGData latest = GetLatestData();
        Logger::GetInstance()->LogInfo("SIPHOGClient: Status - Buffer: " + std::to_string(bufferSize) +
            "/1000, SLED Current: " + std::to_string(latest.sledCurrent) + "mA");
        lastStatusUpdate = now;
    }

    // Ensure data continues flowing to global store even when UI is hidden
    // The polling thread handles this automatically, but we can verify data freshness
    if (m_isConnected && m_isPolling) {
        auto dataAge = std::chrono::duration_cast<std::chrono::seconds>(
            now - GetLatestData().timestamp).count();

        if (dataAge > 10) { // If data is older than 10 seconds, something may be wrong
            Logger::GetInstance()->LogWarning("SIPHOGClient: Data appears stale (age: " +
                std::to_string(dataAge) + "s), connection may have issues");
        }
    }
}