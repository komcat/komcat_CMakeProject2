...
namespace PowerSupply {
    ...
    bool SPDPowerSupply::setVoltage(int channel, double voltage) {
        ...
        if (success) {
            std::cout << "Set CH" << channel << " voltage to " << voltage << "V" << std::endl;
        }
        return success;
    }
    ...
    std::optional<bool> SPDPowerSupply::getOutputState(int channel) const {
        ...
        try {
            ...
            std::cout << "DEBUG: System status: 0x" << std::hex << status
    ...
    bool SPDPowerSupply::setTimeout(int timeout_ms) {
        if (!is_connected_ || !pimpl_->visa_initialized) {
            return false;
        }
    ...
    std::string SPDPowerSupply::query(const std::string& query_cmd) const {
        if (!is_connected_ || !pimpl_->visa_initialized) {
            std::cerr << "DEBUG: Query called but not connected (is_connected_="
                << is_connected_ << ", visa_initialized=" << pimpl_->visa_initialized << ")" << std::endl;
            return "";
        }

        std::cout << "DEBUG: Sending query: " << query_cmd << std::endl;
        ...
        try {
            ...
            std::cout << "DEBUG: Query sent, " << bytes_written << " bytes written" << std::endl;
            ...
            buffer[bytes_read] = '\0';
            std::cout << "DEBUG: Received " << bytes_read << " bytes: " << buffer << std::endl;
