...
namespace UAA3ProcessBuilders {
	...
	std::unique_ptr<SequenceStep> BuildNeedleXYCalibrationSequenceEnhanced_uaa3(
		...
		auto needleAdjustment = std::make_shared<ManualAdjustmentOperation>(
			...
			promptUI,
			false, false, true  // Only Z axis enabled
		);
		...
		sequence->AddOperation(std::make_shared<MoveToPointNameOperation>(
			"gantry-main", "_cam_cal_dot"));
		// 15. REPLACED WITH ManualAdjustmentOperation for camera alignment
		auto cameraAlignment = std::make_shared<ManualAdjustmentOperation>(
			"gantry-main",
			"Camera Crosshair Alignment",
			"Use the jog controls to center the crosshair on the dispensed dot.\n"
			"You can adjust X and Y axes.",
			promptUI,
			true, true, false  // X and Y axes enabled, Z disabled
		);
		cameraAlignment->WithStepSize(0.1)
			.WithShowPosition(true);
		sequence->AddOperation(cameraAlignment);
