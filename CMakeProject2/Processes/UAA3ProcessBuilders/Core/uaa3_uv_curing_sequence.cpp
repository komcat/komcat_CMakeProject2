...
namespace UAA3ProcessBuilders {
  ...
  std::unique_ptr<SequenceStep> BuildUVCuringSequence_uaa3_Configurable(
    ...
    // 1. Move gantry to UV position
    sequence->AddOperation(std::make_shared<MoveToNodeOperation>(
      "gantry-main", "Process_Flow", config.getNode("gantry", "uv-position")));  // was "node_4426"
    ...
    // 3. Toggle UV_PLC1 (pin 14) - Clear output
    sequence->AddOperation(std::make_shared<SetOutputOperation>(
      "IOBottom", config.getInt("io", "uv-plc-pin"), false));  // was 14
    ...
    // 5. Toggle UV_PLC1 (pin 14) - Set output
    sequence->AddOperation(std::make_shared<SetOutputOperation>(
      "IOBottom", config.getInt("io", "uv-plc-pin"), true));  // was 14
    ...
    sequence->AddOperation(std::make_shared<PeriodicMonitorDataValueOperation>(
      "GPIB-Current",
      config.getInt("timing", "uv-curing-ms"),          // was 210000
      config.getInt("timing", "monitor-interval-ms")));  // was 5000
    ...
    // 12. Move hex-left to home
    sequence->AddOperation(std::make_shared<MoveToNodeOperation>(
      "hex-left", "Process_Flow", config.getNode("hex-left", "home")));  // was "node_5480"

    // 13. Move hex-right to home
    sequence->AddOperation(std::make_shared<MoveToNodeOperation>(
      "hex-right", "Process_Flow", config.getNode("hex-right", "home")));  // was "node_5136"

    // 14. Move gantry to safe position
    sequence->AddOperation(std::make_shared<MoveToNodeOperation>(
      "gantry-main", "Process_Flow", config.getNode("gantry", "safe")));  // was "node_4027"
    ...
    sequence->AddOperation(std::make_shared<SetOutputOperation>(
      "IOBottom", config.getInt("io", "vacuum-base-pin"), false));  // was 10
