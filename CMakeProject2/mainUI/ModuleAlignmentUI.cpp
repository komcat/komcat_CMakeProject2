...
// =============================================================================
// NEW: POINTS NAVIGATION SECTION
// =============================================================================

void ModuleAlignmentUI::RenderPointsNavigationSection() {
  ImGui::Text("ProductReference Points Navigation");
  ImGui::Separator();

  if (!m_productReferenceManager) {
    ImGui::TextColored(ImVec4(1, 0, 0, 1), "ProductReferenceManager not available");
    ImGui::Text("Load ProductReference files first in Configuration tab");
    return;
  }
  ...
  if (ImGui::BeginCombo("##PointsProduct",
  ...
  if (m_selectedProductIndex < 0) {
    ImGui::Text("Please select a product to view its points");
    return;
  }

  const auto* product = m_productReferenceManager->GetProductReference(m_selectedProductName);
  if (!product) {
    ImGui::TextColored(ImVec4(1, 0, 0, 1), "Selected product not found");
    return;
  }
  ...
  static int navigationMode = 0;  // 0 = Product Coordinates, 1 = Alignment Coordinates
  ImGui::Text("Navigation Mode:");
  ImGui::RadioButton("Product Coordinates", &navigationMode, 0);
  ImGui::SameLine();
  ImGui::RadioButton("Alignment Coordinates", &navigationMode, 1);
  ImGui::SameLine();
  bool useAlignmentCoordinates = (navigationMode == 1);

  ImGui::TextDisabled("(?)");
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip("Product: Use coordinates from ProductReference file\nAlignment: Transform through alignment system (requires valid alignment)");
  }

  // Check if alignment is available for alignment coordinates
  bool hasValidAlignment = m_moduleAlignment && m_moduleAlignment->HasValidAlignment();
  if (useAlignmentCoordinates && !hasValidAlignment) {
    ImGui::TextColored(ImVec4(1, 0, 0, 1), "Alignment coordinates require a valid alignment");
    ImGui::Text("Complete an alignment first or switch to Product coordinates");
    return;
  }
  ...
  // Points table
  ImGui::Text("Points in %s:", product->name.c_str());

  if (ImGui::BeginTable("PointsTable", 7, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY, ImVec2(0, 300))) {
    ...
    ImGui::TableSetupColumn("Z", ImGuiTableColumnFlags_WidthFixed, 60);
    ImGui::TableSetupColumn("Valid", ImGuiTableColumnFlags_WidthFixed, 50);
    ImGui::TableSetupColumn("Action", ImGuiTableColumnFlags_WidthStretch);
    ImGui::TableHeadersRow();

    // Table rows
    for (const auto& point : product->points) {
      // Skip fiducial points
      ...
      std::string typeStr = GetActionTypeString(point.actionType == ProductReferenceManager::Point3D::ActionType::FIDUCIAL ? "Fiducial" :
        point.actionType == ProductReferenceManager::Point3D::ActionType::INSPECTION ? "Inspection" : "Unknown");
      ImVec4 typeColor = (point.actionType == ProductReferenceManager::Point3D::ActionType::FIDUCIAL) ?
        ImVec4(0, 1, 0, 1) : ImVec4(0, 0.8f, 1, 1);
      ImGui::TextColored(typeColor, "%s", typeStr.c_str());
      ...
      ImGui::TextColored(point.isValid ? ImVec4(0, 1, 0, 1) : ImVec4(1, 0, 0, 1),
        point.isValid ? "✓" : "✗");

      // Navigation button
      ImGui::TableSetColumnIndex(6);

      // Check if system is ready for movement
      bool canNavigate = m_moduleAlignment && m_moduleAlignment->IsReadyForAlignment() && point.isValid;

      if (!canNavigate) {
        ImGui::BeginDisabled();
      }

      std::string buttonLabel = "Go to##" + point.name;
      if (ImGui::Button(buttonLabel.c_str(), ImVec2(60, 0))) {
        if (useAlignmentCoordinates) {
          NavigateToAlignmentCoordinate(point.x, point.y, point.z);
        }
        else {
          NavigateToPoint(m_selectedProductName, point.name);
        }
      }
      ...
      // Show why navigation is disabled
      if (!canNavigate && ImGui::IsItemHovered()) {
        if (!point.isValid) {
          ImGui::SetTooltip("Point is marked as invalid");
        }
        else if (!m_moduleAlignment || !m_moduleAlignment->IsReadyForAlignment()) {
          ImGui::SetTooltip("System not ready for movement");
        }
      }
    }
  ...
  if (!m_lastMoveStatus.empty()) {
    ...
    ImGui::TextColored(statusColor, "Last Navigation: %s", m_lastMoveStatus.c_str());
  }
  ...
  if (ImGui::Button("Visit All Fiducials", ImVec2(130, 0))) {
    // Could implement sequential navigation to all fiducial points
    ImGui::OpenPopup("Visit All Fiducials");
  }

  ImGui::SameLine();
  if (ImGui::Button("Visit All Inspection", ImVec2(130, 0))) {
    // Could implement sequential navigation to all inspection points
    ImGui::OpenPopup("Visit All Inspection");
  }

  // Popup dialogs for batch operations
  if (ImGui::BeginPopupModal("Visit All Fiducials", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
    ImGui::Text("This would navigate to all fiducial points sequentially.");
  ...
  if (ImGui::BeginPopupModal("Visit All Inspection", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
    ImGui::Text("This would navigate to all inspection points sequentially.");
    ImGui::Text("Feature coming soon!");
  ...
  void ModuleAlignmentUI::RenderUI() {
    ...
    if (ImGui::BeginTabBar("AlignmentTabs")) {
      ...
      // NEW: Points Navigation Tab
      if (ImGui::BeginTabItem("Points Navigation", nullptr, (m_activeTab == 4) ? ImGuiTabItemFlags_SetSelected : 0)) {
        if (ImGui::IsItemActive()) m_activeTab = 4;
        RenderPointsNavigationSection();
        ImGui::EndTabItem();
      }

      if (ImGui::BeginTabItem("Advanced", nullptr, (m_activeTab == 5) ? ImGuiTabItemFlags_SetSelected : 0)) {
        if (ImGui::IsItemActive()) m_activeTab = 5;
  ...
  void ModuleAlignmentUI::NavigateToPoint(const std::string & productName, const std::string & pointName) {
    if (!m_productReferenceManager || !m_moduleAlignment) {
      m_lastMoveStatus = "System not available";
      m_lastMoveSuccess = false;
      return;
    }

    const auto* point = m_productReferenceManager->GetPoint(productName, pointName);
    if (!point) {
      m_lastMoveStatus = "Point not found: " + pointName;
      m_lastMoveSuccess = false;
      return;
    }

    if (!point->isValid) {
      m_lastMoveStatus = "Point marked as invalid: " + pointName;
      m_lastMoveSuccess = false;
      return;
    }

    // Move directly to the point's coordinates from ProductReference
    bool success = m_moduleAlignment->MoveToLocalCoordinate(
      point->x,
      point->y,
      point->z,
    ...
    if (success) {
      m_lastMoveStatus = "Navigated to " + pointName + " (" +
        std::to_string(point->x) + ", " +
        std::to_string(point->y) + ", " +
        std::to_string(point->z) + ")";

      std::cout << "ModuleAlignmentUI: Successfully navigated to point " << pointName
        << " at (" << point->x << ", " << point->y << ", " << point->z << ")" << std::endl;
    }
    else {
      m_lastMoveStatus = "Navigation failed to " + pointName + ": " + m_moduleAlignment->GetLastError();
      std::cout << "ModuleAlignmentUI: Navigation failed: " << m_moduleAlignment->GetLastError() << std::endl;
    }
  }

  void ModuleAlignmentUI::NavigateToAlignmentCoordinate(double x, double y, double z) {
    if (!m_moduleAlignment) {
      m_lastMoveStatus = "ModuleAlignment not available";
      m_lastMoveSuccess = false;
      return;
    }

    if (!m_moduleAlignment->HasValidAlignment()) {
      m_lastMoveStatus = "No valid alignment available";
      m_lastMoveSuccess = false;
      return;
    }

    // Move using alignment coordinate system
    bool success = m_moduleAlignment->MoveToLocalCoordinate(
      x, y, z,
      m_deviceName,
      m_waitForCompletion
    ...
    if (success) {
      m_lastMoveStatus = "Moved to alignment coordinates (" +
        std::to_string(x) + ", " +
        std::to_string(y) + ", " +
        std::to_string(z) + ")";

      std::cout << "ModuleAlignmentUI: Successfully moved to alignment coordinates ("
        << x << ", " << y << ", " << z << ")" << std::endl;
    }
    else {
      m_lastMoveStatus = "Alignment navigation failed: " + m_moduleAlignment->GetLastError();
      std::cout << "ModuleAlignmentUI: Alignment navigation failed: " << m_moduleAlignment->GetLastError() << std::endl;
    }
  }

  std::string ModuleAlignmentUI::GetActionTypeString(const std::string & actionType) {
    if (actionType == "Fiducial") return "Fiducial";
    if (actionType == "Inspection") return "Inspect";
    return actionType;
  }