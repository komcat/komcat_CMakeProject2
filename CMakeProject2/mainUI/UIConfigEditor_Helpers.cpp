// UIConfigEditor_Helpers.cpp - Helper methods and clipboard functionality
...
void UIConfigEditor::RenderClipboardConfirmationPopup() {
  ...
  if (ImGui::BeginPopupModal("Confirm Position Update", &isOpen, ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (ImGui::BeginTable("PositionValuesTable", 4, ImGuiTableFlags_Borders)) {
      ...
      ImVec4 unchangedColor = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);

      auto addRow = [&](const char* axis, double oldVal, double newVal) {
        ImGui::TableNextRow();
        ImGui::TableNextColumn(); ImGui::Text("%s", axis);
        ImGui::TableNextColumn(); ImGui::Text("%.6f", oldVal);
        ImGui::TableNextColumn(); ImGui::Text("%.6f", newVal);
