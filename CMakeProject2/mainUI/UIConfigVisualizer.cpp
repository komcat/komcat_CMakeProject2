...
void UIConfigVisualizer::SetMachineOperations(MachineOperations* machineOps) {
	// Unsubscribe from previous controllers if any
	if (m_machineOperations && m_positionSubscriber) {
		// TODO: Unsubscribe from controllers
		m_positionSubscriber->ClearAllDevices();
	}
	...
	if (m_machineOperations) {
		...
		// Subscribe to controllers for position updates
		SubscribeToControllers();
	}
	else {
...
void UIConfigVisualizer::InitializePositionSubscriber() {
	if (!m_positionSubscriber) {
		m_positionSubscriber = std::make_shared<UIConfigVisualizerPositionSubscriber>();
		m_logger->LogInfo("UIConfigVisualizer: Position subscriber initialized");
	}
}

void UIConfigVisualizer::SubscribeToControllers() {
	if (!m_positionSubscriber || !m_machineOperations) {
		return;
	}
	...
	// Subscribe to PI controllers
	PIControllerManager* piManager = m_machineOperations->GetPIControllerManager();
	if (piManager) {
		std::vector<std::string> piDevices = { "hex-left", "hex-right", "gantry-main" };

		for (const auto& deviceName : piDevices) {
			PIController* controller = piManager->GetController(deviceName);
			if (controller && controller->IsConnected()) {
				std::string subscriberId = "UIConfigVisualizer_" + deviceName;
				controller->SubscribeToPositions(m_positionSubscriber.get(), subscriberId);
				subscribed++;
				m_logger->LogInfo("UIConfigVisualizer: Subscribed to PI controller " + deviceName);
			}
			else {
				m_logger->LogInfo("UIConfigVisualizer: PI controller " + deviceName +
					" not found or not connected");
			}
		}
	}

	// Subscribe to ACS controllers if available
	// Note: You'll need to add GetACSControllerManager() to MachineOperations
	
	ACSControllerManager* acsManager = m_machineOperations->GetACSControllerManager();
	if (acsManager) {
			std::vector<std::string> acsDevices = {"gantry-main"};

			for (const auto& deviceName : acsDevices) {
					ACSController* controller = acsManager->GetController(deviceName);
					if (controller && controller->IsConnected()) {
							std::string subscriberId = "UIConfigVisualizer_" + deviceName;
							controller->SubscribeToPositions(m_positionSubscriber.get(), subscriberId);
							subscribed++;
							m_logger->LogInfo("UIConfigVisualizer: Subscribed to ACS controller " + deviceName);
					}
			}
	}
	

	m_logger->LogInfo("UIConfigVisualizer: Subscribed to " +
		std::to_string(subscribed) + " controllers total");
}

void UIConfigVisualizer::CleanupPositionSubscriber() {
	if (m_positionSubscriber && m_machineOperations) {
		// Unsubscribe from all controllers
		PIControllerManager* piManager = m_machineOperations->GetPIControllerManager();
		if (piManager) {
			// Unsubscribe from each controller
			std::vector<std::string> deviceNames = { "hex-left", "hex-right", "gantry-main" };

			for (const auto& deviceName : deviceNames) {
				PIController* controller = piManager->GetController(deviceName);
				if (controller) {
					std::string subscriberId = "UIConfigVisualizer_" + deviceName;
					controller->UnsubscribeFromPositions(subscriberId);
					m_logger->LogInfo("UIConfigVisualizer: Unsubscribed from " + deviceName);
				}
			}
		}
	...
	m_positionSubscriber.reset();
	m_logger->LogInfo("UIConfigVisualizer: Position subscriber cleaned up");
}